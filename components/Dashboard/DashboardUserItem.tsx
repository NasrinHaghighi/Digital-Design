...
function DashboardUserItem({item}:{item:User}) {
  ...
  return (
    ...
    <div className='flex flex-col lg:flex-row justify-between text-sm mb-5 gap-y-3 '>
                    ...
                    <div>ایمیل :<span className='px-1 text-yellow-700 font-semibold'>{item?.email}</span></div>

                    
                   

  </div>
  <div> سمت کاربر :<span className='text-red-400 font-semibold'>{item.role}</span> </div>
